...
from .models import Fixture, Franchise, MatchPoint, Player, Tournament
...
@lru_cache(maxsize=32)
def get_franchise(request) -> pd.DataFrame:
    # Read `franchise` column from franchise table
    return pd.DataFrame.from_records(Franchise.objects.filter(user=request.user))
...
def prepare_final_match_point_df(request) -> None:
    ...
    for match_scorecard in match_scorecards:
        ...
        for index, row in merged_df.iterrows():
            ...
            match_point_model.match_date = yesterday
            match_point_model.tournament = Tournament.objects.get(tournament="ODIWC2023")  # TODO: Add tournament name
            # Iterates over each row of the merged dataframe
